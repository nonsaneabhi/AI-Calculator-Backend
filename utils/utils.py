...
def analyze_image(img: Image, variables: dict):
    ...
    prompt = (
        ...
        f"Here is a dictionary of user-assigned variables. If the given expression has any of these variables, use its actual value from this dictionary accordingly: {variables}. "
    ...
    answers = []
    try:
        cleaned_response = clean_and_parse_json(response.text)
...
def clean_and_parse_json(response_text):
    ...
    json_like_text = re.search(r'\[.*\]', response_text)

    if json_like_text:
        try:
            cleaned_text = json_like_text.group(0)
